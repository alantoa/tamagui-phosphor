import React, { memo } from 'react'
import PropTypes from 'prop-types'
import type { NamedExoticComponent } from 'react'
import type { IconProps } from '@tamagui/helpers-icon'
import {
  Svg,
  Circle as _Circle,
  Ellipse as SvgEllipse,
  G,
  LinearGradient,
  RadialGradient,
  Line,
  Path as SvgPath,
  Polygon as SvgPolygon,
  Polyline as SvgPolyline,
  Rect as SvgRect,
  Symbol as SvgSymbol,
  Text as _Text,
  Use,
  Defs,
  Stop as SvgStop,
} from 'react-native-svg'
import { themed } from '@tamagui/helpers-icon'

const Icon = (props) => {
  const { color = 'black', size = 24, ...otherProps } = props
  return (
    <Svg viewBox="0 0 256 256" {...otherProps}>
      <SvgRect width="256" height="256" fill="none" />
      <SvgPolygon
        points="208 216 128 201.46 128 201.46 128 144 208 144 208 216"
        opacity="0.2"
      />
      <SvgPolygon points="96 195.64 32 184 32 144 96 144 96 195.64" opacity="0.2" />
      <SvgPolygon
        points="208 40 128 54.55 128 54.55 128 112 208 112 208 40"
        opacity="0.2"
      />
      <SvgPolygon points="96 60.36 32 72 32 112 96 112 96 60.36" opacity="0.2" />
      <SvgPolygon
        points="208 216 128 201.46 128 201.46 128 144 208 144 208 216"
        fill="none"
        stroke={color}
        strokeLinecap="round"
        strokeLinejoin="round"
        strokeWidth="16"
      />
      <SvgPolygon
        points="96 195.64 32 184 32 144 96 144 96 195.64"
        fill="none"
        stroke={color}
        strokeLinecap="round"
        strokeLinejoin="round"
        strokeWidth="16"
      />
      <SvgPolygon
        points="208 40 128 54.55 128 54.55 128 112 208 112 208 40"
        fill="none"
        stroke={color}
        strokeLinecap="round"
        strokeLinejoin="round"
        strokeWidth="16"
      />
      <SvgPolygon
        points="96 60.36 32 72 32 112 96 112 96 60.36"
        fill="none"
        stroke={color}
        strokeLinecap="round"
        strokeLinejoin="round"
        strokeWidth="16"
      />
    </Svg>
  )
}

Icon.displayName = 'WindowsLogoDuotone'

export const WindowsLogoDuotone: NamedExoticComponent<IconProps> = memo<IconProps>(
  themed(Icon)
)
